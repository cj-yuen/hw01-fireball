...
export class Shader {
  ...
  constructor(type: number, source: string, translucent: boolean = false) {
...
class ShaderProgram {
  ...
  setFireballStrength(strength: number) {
    this.use();
  ...
  setFireballOctaves(octaves: number) {
    this.use();
  ...
  setFireballAlpha(alpha: number) {
    this.use();
    if (this.unifAlpha !== -1) {
      gl.uniform1f(this.unifAlpha, alpha);
    }
  }
  ...
  setMousePos(mouseX: number, mouseY: number) {
    ...
    if (this.unifMousePos !== -1) {
      gl.uniform2f(this.unifMousePos, mouseX, mouseY);
    }
  }
